	...
	public void listShow() {
		...
		task.setOnSucceeded(e -> {
			...
			countLabel.setText("Number of return records = " + String
					.valueOf(DataSourceAddress.getInstance().searchAddressWardCountryCount(Main.ward, Main.country)));
		});
		task.setOnFailed(e -> {
			progressBar.setVisible(false);
...
class SearchAddressWardCountry extends Task<ObservableList<Address>> {
	...
	public ObservableList<Address> call() throws Exception {
		return FXCollections.observableArrayList(
				(DataSourceAddress.getInstance().searchAddressWardCountry(Main.ward, Main.country)));
	}
}
