...
public class AddressSearchCountryController {
	@FXML
	BorderPane mainPane;
	@FXML
	...
	public void listShow() {
		...
		countLabel.setText("");
		Task<ObservableList<Address>> task = new SearchAddressCountry();
		...
		task.setOnSucceeded(e -> {
			...
			countLabel.setText("Number of return records = "
					+ String.valueOf(DataSourceAddress.getInstance().searchAddressCountryCount(Main.country)));
		});
		task.setOnFailed(e -> {
			progressBar.setVisible(false);
...
class SearchAddressCountry extends Task<ObservableList<Address>> {
	@Override
	public ObservableList<Address> call() throws Exception {
		return FXCollections.observableArrayList((DataSourceAddress.getInstance().searchAddressCountry(Main.country)));
	}
}
