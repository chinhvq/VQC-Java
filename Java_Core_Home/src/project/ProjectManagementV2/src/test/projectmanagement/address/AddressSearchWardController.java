...
public class AddressSearchWardController {
	@FXML
	BorderPane mainPane;
	@FXML
	...
	public void listShow() {
		...
		countLabel.setText("");
		Task<ObservableList<Address>> task = new SearchAddressWard();
		...
		task.setOnSucceeded(e -> {
			...
			countLabel.setText("Number of return records = "
					+ String.valueOf(DataSourceAddress.getInstance().searchAddressWardCount(Main.ward)));
		});
		task.setOnFailed(e -> {
			progressBar.setVisible(false);
...
class SearchAddressWard extends Task<ObservableList<Address>> {
	@Override
	public ObservableList<Address> call() throws Exception {
		return FXCollections.observableArrayList((DataSourceAddress.getInstance().searchAddressWard(Main.ward)));
	}
}
