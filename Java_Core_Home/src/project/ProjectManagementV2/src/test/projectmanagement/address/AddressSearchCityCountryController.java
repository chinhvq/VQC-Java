	...
	public void listShow() {
		...
		task.setOnSucceeded(e -> {
			...
			countLabel.setText("Number of return records = "
					+ String.valueOf(
							DataSourceAddress.getInstance().searchAddressCityCountryCount(Main.city, Main.country)));
		});
		task.setOnFailed(e -> {
			progressBar.setVisible(false);
...
class SearchAddressCityCountry extends Task<ObservableList<Address>> {
	...
	public ObservableList<Address> call() throws Exception {
		return FXCollections.observableArrayList((DataSourceAddress.getInstance().searchAddressCityCountry(Main.city, Main.country)));
	}
}
