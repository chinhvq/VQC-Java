...
public class ProjectSearchNameController {
	@FXML
	BorderPane mainPane;
	@FXML
	...
	public void listShow() {
		...
		countLabel.setText("");
		Task<ObservableList<Project>> task = new SearchProjectName();
		...
		task.setOnSucceeded(e -> {
			...
			countLabel.setText("Number of return records = "
					+ String.valueOf(DataSourceProject.getInstance().searchProjectNameCount(Main.projectName)));
		});
		task.setOnFailed(e -> {
			progressBar.setVisible(false);
...
class SearchProjectName extends Task<ObservableList<Project>> {
	@Override
	public ObservableList<Project> call() throws Exception {
		return FXCollections.observableArrayList((DataSourceProject.getInstance().searchProjectName(Main.projectName)));
	}
}
