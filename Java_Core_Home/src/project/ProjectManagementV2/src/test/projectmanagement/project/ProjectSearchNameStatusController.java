...
public class ProjectSearchNameStatusController {
	@FXML
	BorderPane mainPane;
	@FXML
	...
	public void listShow() {
		...
		countLabel.setText("");
		Task<ObservableList<Project>> task = new SearchProjectNameStatus();
		...
		task.setOnSucceeded(e -> {
			...
			countLabel.setText("Number of return records = " + String.valueOf(
					DataSourceProject.getInstance().searchProjectNameStatusCount(Main.projectName, Main.projectStatus)));
		});
		task.setOnFailed(e -> {
			progressBar.setVisible(false);
...
class SearchProjectNameStatus extends Task<ObservableList<Project>> {
	@Override
	public ObservableList<Project> call() throws Exception {
		return FXCollections.observableArrayList(
				(DataSourceProject.getInstance().searchProjectNameStatus(Main.projectName, Main.projectStatus)));
	}
}
