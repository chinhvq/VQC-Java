...
public class ProjectSearchCategoryController {
	@FXML
	BorderPane mainPane;
	@FXML
	...
	public void listShow() {
		...
		countLabel.setText("");
		Task<ObservableList<Project>> task = new SearchProjectCategory();
		...
		task.setOnSucceeded(e -> {
			...
			countLabel.setText("Number of return records = "
					+ String.valueOf(DataSourceProject.getInstance().searchProjectCategoryCount(Main.projectCategory)));
		});
		task.setOnFailed(e -> {
			progressBar.setVisible(false);
...
class SearchProjectCategory extends Task<ObservableList<Project>> {
	@Override
	public ObservableList<Project> call() throws Exception {
		return FXCollections.observableArrayList((DataSourceProject.getInstance().searchProjectCategory(Main.projectCategory)));
	}
}
