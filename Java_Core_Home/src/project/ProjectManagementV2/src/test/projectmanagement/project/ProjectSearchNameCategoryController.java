...
public class ProjectSearchNameCategoryController {
	@FXML
	BorderPane mainPane;
	@FXML
	...
	public void listShow() {
		...
		countLabel.setText("");
		Task<ObservableList<Project>> task = new SearchProjectNameCategory();
		...
		task.setOnSucceeded(e -> {
			...
			countLabel.setText("Number of return records = " + String.valueOf(
					DataSourceProject.getInstance().searchProjectNameCategoryCount(Main.projectName, Main.projectCategory)));
		});
		task.setOnFailed(e -> {
			progressBar.setVisible(false);
...
class SearchProjectNameCategory extends Task<ObservableList<Project>> {
	@Override
	public ObservableList<Project> call() throws Exception {
		return FXCollections.observableArrayList(
				(DataSourceProject.getInstance().searchProjectNameCategory(Main.projectName, Main.projectCategory)));
	}
}
