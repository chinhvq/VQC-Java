...
public class EmployeeSearchGenderTitleController {
	@FXML
	BorderPane mainPane;
	@FXML
	...
	public void listShow() {
		...
		countLabel.setText("");
		Task<ObservableList<Employee>> task = new SearchEmployeeGenderTitle();
		...
		task.setOnSucceeded(e -> {
			...
			countLabel.setText("Number of return records = "
					+ String.valueOf(DataSourceEmployee.getInstance().searchEmployeeGenderTitleCount(Main.gender, Main.title)));
		});
		task.setOnFailed(e -> {
			progressBar.setVisible(false);
...
class SearchEmployeeGenderTitle extends Task<ObservableList<Employee>> {
	@Override
	public ObservableList<Employee> call() throws Exception {
		return FXCollections.observableArrayList((DataSourceEmployee.getInstance().searchEmployeeGenderTitle(Main.gender, Main.title)));
	}
}
