...
public class EmployeeSearchFirstNameController {
	@FXML
	BorderPane mainPane;
	@FXML
	...
	public void listShow() {
		...
		countLabel.setText("");
		Task<ObservableList<Employee>> task = new SearchEmployeeFirstName();
		...
		task.setOnSucceeded(e -> {
			...
			countLabel.setText("Number of return records = "
					+ String.valueOf(DataSourceEmployee.getInstance().searchEmployeeFirstNameCount(Main.firstName)));
		});
		task.setOnFailed(e -> {
			progressBar.setVisible(false);
...
class SearchEmployeeFirstName extends Task<ObservableList<Employee>> {
	@Override
	public ObservableList<Employee> call() throws Exception {
		return FXCollections.observableArrayList((DataSourceEmployee.getInstance().searchEmployeeFirstName(Main.firstName)));
	}
}
