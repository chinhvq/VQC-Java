...
public class EmployeeSearchGenderAgeController {
	@FXML
	BorderPane mainPane;
	@FXML
	...
	public void listShow() {
		...
		countLabel.setText("");
		Task<ObservableList<Employee>> task = new SearchEmployeeGenderAge();
		...
		task.setOnSucceeded(e -> {
			...
			countLabel.setText("Number of return records = "
					+ String.valueOf(DataSourceEmployee.getInstance().searchEmployeeGenderAgeCount(Main.gender, Main.age)));
		});
		task.setOnFailed(e -> {
			progressBar.setVisible(false);
...
class SearchEmployeeGenderAge extends Task<ObservableList<Employee>> {
	@Override
	public ObservableList<Employee> call() throws Exception {
		return FXCollections.observableArrayList((DataSourceEmployee.getInstance().searchEmployeeGenderAge(Main.gender, Main.age)));
	}
}
