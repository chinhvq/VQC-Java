	...
	public void listShow() {
		...
		task.setOnSucceeded(e -> {
			...
			countLabel.setText("Number of return records = " + String
					.valueOf(DataSourceEmployee.getInstance().searchEmployeeLastNameGenderCount(Main.lastname, Main.gender)));
		});
		task.setOnFailed(e -> {
			progressBar.setVisible(false);
...
class SearchEmployeeLastNameGender extends Task<ObservableList<Employee>> {
	...
	public ObservableList<Employee> call() throws Exception {
		return FXCollections.observableArrayList(
				(DataSourceEmployee.getInstance().searchEmployeeLastNameGender(Main.lastname, Main.gender)));
	}
}
