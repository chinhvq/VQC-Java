	...
	public void listShow() {
		...
		task.setOnSucceeded(e -> {
			...
			countLabel.setText("Number of return records = "
					+ String.valueOf(DataSourceCustomer.getInstance().searchCustomerBusinesTypeCount(Main.businessType)));
		});
		task.setOnFailed(e -> {
			progressBar.setVisible(false);
...
class SearchCustomerBusinesType extends Task<ObservableList<Customer>> {
	...
	public ObservableList<Customer> call() throws Exception {
		return FXCollections.observableArrayList((DataSourceCustomer.getInstance().searchCustomerBusinesType(Main.businessType)));
	}
}
