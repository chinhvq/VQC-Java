...
public class CustomerSearchCustomerNameController {
	@FXML
	BorderPane mainPane;
	@FXML
	...
	public void listShow() {
		...
		countLabel.setText("");
		Task<ObservableList<Customer>> task = new SearchCustomerName();
		...
		task.setOnSucceeded(e -> {
			...
			countLabel.setText("Number of return records = "
					+ String.valueOf(DataSourceCustomer.getInstance().searchCustomerNameCount(Main.customerName)));
		});
		task.setOnFailed(e -> {
			progressBar.setVisible(false);
		});
...
class SearchCustomerName extends Task<ObservableList<Customer>> {
	@Override
	public ObservableList<Customer> call() throws Exception {
		return FXCollections.observableArrayList((DataSourceCustomer.getInstance().searchCustomerName(Main.customerName)));
	}
}
