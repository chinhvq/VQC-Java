	...
	public void listShow() {
		...
		task.setOnSucceeded(e -> {
			...
			countLabel.setText("Number of return records = "
					+ String.valueOf(DataSource.getInstance().searchProjectNameCount(Main.projectName)));
		});
		task.setOnFailed(e -> {
			progressBar.setVisible(false);
...
class SearchProjectName extends Task<ObservableList<Project>> {
	...
	public ObservableList<Project> call() throws Exception {
		return FXCollections.observableArrayList((DataSource.getInstance().searchProjectName(Main.projectName)));
	}
}
