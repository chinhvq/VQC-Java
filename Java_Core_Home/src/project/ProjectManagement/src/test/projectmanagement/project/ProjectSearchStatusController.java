	...
	public void listShow() {
		...
		task.setOnSucceeded(e -> {
			...
			countLabel.setText("Number of return records = "
					+ String.valueOf(DataSource.getInstance().searchProjectStatusCount(Main.projectStatus)));
		});
		task.setOnFailed(e -> {
			progressBar.setVisible(false);
...
class SearchProjectStatus extends Task<ObservableList<Project>> {
	...
	public ObservableList<Project> call() throws Exception {
		return FXCollections.observableArrayList((DataSource.getInstance().searchProjectStatus(Main.projectStatus)));
	}
}
