	...
	public void listShow() {
		...
		task.setOnSucceeded(e -> {
			...
			countLabel.setText("Number of return records = "
					+ String.valueOf(DataSource.getInstance().queryDepartmentSearchCount(Main.deptID, Main.deptName)));
		});
		task.setOnFailed(e -> {
			progressBar.setVisible(false);
...
class ListResultDepartment extends Task<ObservableList<Department>> {
	...
	public ObservableList<Department> call() throws Exception {
		return FXCollections
				.observableArrayList((DataSource.getInstance().searchDepartmentIDName(Main.deptID, Main.deptName)));
	}
