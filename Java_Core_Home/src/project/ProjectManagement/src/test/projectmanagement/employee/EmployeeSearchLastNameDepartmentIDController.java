	...
	public void listShow() {
		...
		task.setOnSucceeded(e -> {
			...
			countLabel.setText("Number of return records = "
					+ String.valueOf(DataSource.getInstance().searchEmployeeLastNameDepartmentIDCount(Main.lastname, Main.departmentID)));
		});
		task.setOnFailed(e -> {
			progressBar.setVisible(false);
...
class SearchEmployeeLastNameDepartmentID extends Task<ObservableList<Employee>> {
	...
	public ObservableList<Employee> call() throws Exception {
		return FXCollections.observableArrayList((DataSource.getInstance().searchEmployeeLastNameDepartmentID(Main.lastname, Main.departmentID)));
	}
}
