	...
	public void listShow() {
		...
		task.setOnSucceeded(e -> {
			...
			countLabel.setText("Number of return records = "
					+ String.valueOf(DataSource.getInstance().searchEmployeeLastNameAgeCount(Main.lastname, Main.age)));
		});
		task.setOnFailed(e -> {
			progressBar.setVisible(false);
...
class SearchEmployeeLastNameAge extends Task<ObservableList<Employee>> {
	...
	public ObservableList<Employee> call() throws Exception {
		return FXCollections
				.observableArrayList((DataSource.getInstance().searchEmployeeLastNameAge(Main.lastname, Main.age)));
	}
}
