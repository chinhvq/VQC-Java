...
public class EmployeeSearchPhoneNumberController {
	@FXML
	BorderPane mainPane;
	@FXML
	...
	public void listShow() {
		...
		countLabel.setText("");
		Task<ObservableList<Employee>> task = new SearchEmployeePhoneNumber();
		...
		task.setOnSucceeded(e -> {
			...
			countLabel.setText("Number of return records = "
					+ String.valueOf(DataSource.getInstance().searchEmployeePhoneNumberCount(Main.phonenumber)));
		});
		task.setOnFailed(e -> {
			progressBar.setVisible(false);
...
class SearchEmployeePhoneNumber extends Task<ObservableList<Employee>> {
	@Override
	public ObservableList<Employee> call() throws Exception {
		return FXCollections.observableArrayList((DataSource.getInstance().searchEmployeePhoneNumber(Main.phonenumber)));
	}
}
