	...
	public void listShow() {
		...
		task.setOnSucceeded(e -> {
			...
			countLabel.setText("Number of return records = "
					+ String.valueOf(DataSource.getInstance().searchEmployeeGenderAgeCount(Main.gender, Main.age)));
		});
		task.setOnFailed(e -> {
			progressBar.setVisible(false);
...
class SearchEmployeeGenderAge extends Task<ObservableList<Employee>> {
	...
	public ObservableList<Employee> call() throws Exception {
		return FXCollections.observableArrayList((DataSource.getInstance().searchEmployeeGenderAge(Main.gender, Main.age)));
	}
}
