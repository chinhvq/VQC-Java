	...
	public void listShow() {
		...
		task.setOnSucceeded(e -> {
			...
			countLabel.setText("Number of return records = " + String
					.valueOf(DataSource.getInstance().searchEmployeeAgeDepartmentIDCount(Main.age, Main.departmentID)));
		});
		task.setOnFailed(e -> {
			progressBar.setVisible(false);
...
class SearchEmployeeAgeDepartmentID extends Task<ObservableList<Employee>> {
	...
	public ObservableList<Employee> call() throws Exception {
		return FXCollections.observableArrayList(
				(DataSource.getInstance().searchEmployeeAgeDepartmentID(Main.age, Main.departmentID)));
	}
}
