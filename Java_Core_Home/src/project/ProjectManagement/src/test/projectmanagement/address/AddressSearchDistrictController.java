...
public class AddressSearchDistrictController {
	@FXML
	BorderPane mainPane;
	@FXML
	...
	public void listShow() {
		...
		countLabel.setText("");
		Task<ObservableList<Address>> task = new SearchAddressDistrict();
		...
		task.setOnSucceeded(e -> {
			...
			countLabel.setText("Number of return records = "
					+ String.valueOf(DataSource.getInstance().searchAddressDistrictCount(Main.district)));
		});
		task.setOnFailed(e -> {
			progressBar.setVisible(false);
...
class SearchAddressDistrict extends Task<ObservableList<Address>> {
	@Override
	public ObservableList<Address> call() throws Exception {
		return FXCollections.observableArrayList((DataSource.getInstance().searchAddressDistrict(Main.district)));
	}
}
