	...
	public void listShow() {
		...
		task.setOnSucceeded(e -> {
			...
			countLabel.setText("Number of return records = " + String
					.valueOf(DataSource.getInstance().searchAddressWardDistrictCount(Main.ward, Main.district)));
		});
		task.setOnFailed(e -> {
			progressBar.setVisible(false);
...
class SearchAddressWardDistrict extends Task<ObservableList<Address>> {
	...
	public ObservableList<Address> call() throws Exception {
		return FXCollections
				.observableArrayList((DataSource.getInstance().searchAddressWardDistrict(Main.ward, Main.district)));
	}
}
