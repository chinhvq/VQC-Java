	...
	public void listShow() {
		...
		task.setOnSucceeded(e -> {
			...
			countLabel.setText("Number of return records = "
					+ String.valueOf(DataSource.getInstance().searchAddressCityCount(Main.city)));
		});
		task.setOnFailed(e -> {
			progressBar.setVisible(false);
...
class SearchAddressCity extends Task<ObservableList<Address>> {
	...
	public ObservableList<Address> call() throws Exception {
		return FXCollections.observableArrayList((DataSource.getInstance().searchAddressCity(Main.city)));
	}
}
