	...
	public void listShow() {
		...
		task.setOnSucceeded(e -> {
			...
			countLabel.setText("Number of return records = "
					+ String.valueOf(DataSource.getInstance().searchAddressWardCityCount(Main.ward, Main.city)));
		});
		task.setOnFailed(e -> {
			progressBar.setVisible(false);
...
class SearchAddressWardCity extends Task<ObservableList<Address>> {
	...
	public ObservableList<Address> call() throws Exception {
		return FXCollections
				.observableArrayList((DataSource.getInstance().searchAddressWardCity(Main.ward, Main.city)));
	}
}
