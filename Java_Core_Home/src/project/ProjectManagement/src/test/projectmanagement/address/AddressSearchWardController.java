	...
	public void listShow() {
		...
		task.setOnSucceeded(e -> {
			...
			countLabel.setText("Number of return records = "
					+ String.valueOf(DataSource.getInstance().searchAddressWardCount(Main.ward)));
		});
		task.setOnFailed(e -> {
			progressBar.setVisible(false);
...
class SearchAddressWard extends Task<ObservableList<Address>> {
	...
	public ObservableList<Address> call() throws Exception {
		return FXCollections.observableArrayList((DataSource.getInstance().searchAddressWard(Main.ward)));
	}
}
