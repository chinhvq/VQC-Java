...
public class AddressSearchCityCountryController {
	@FXML
	BorderPane mainPane;
	@FXML
	...
	public void listShow() {
		...
		countLabel.setText("");
		Task<ObservableList<Address>> task = new SearchAddressCityCountry();
		...
		task.setOnSucceeded(e -> {
			...
			countLabel.setText("Number of return records = "
					+ String.valueOf(DataSource.getInstance().searchAddressCityCountryCount(Main.city, Main.country)));
		});
		task.setOnFailed(e -> {
			progressBar.setVisible(false);
...
class SearchAddressCityCountry extends Task<ObservableList<Address>> {
	@Override
	public ObservableList<Address> call() throws Exception {
		return FXCollections.observableArrayList((DataSource.getInstance().searchAddressCityCountry(Main.city, Main.country)));
	}
}
