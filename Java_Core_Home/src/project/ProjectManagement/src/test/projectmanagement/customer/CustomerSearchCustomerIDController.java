	...
	public void listShow() {
		...
		task.setOnSucceeded(e -> {
			...
			countLabel.setText("Number of return records = "
					+ String.valueOf(DataSource.getInstance().searchCustomerIDCount(Main.CustomerID)));
		});
		task.setOnFailed(e -> {
			progressBar.setVisible(false);
...
class SearchCustomerID extends Task<ObservableList<Customer>> {
	...
	public ObservableList<Customer> call() throws Exception {
		return FXCollections.observableArrayList((DataSource.getInstance().searchCustomerID(Main.CustomerID)));
	}
}
